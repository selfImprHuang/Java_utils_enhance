...
!(async () => {
  ...
  message = "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>全民开红包</font> \n\n"
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr && cookiesArr.length >= 2) {
      ...
      for (let j = 0; j < redPacketId.length && $.canHelp; j++) {
        ...
        $.max = false;
        message += "<font color=\'#FFA500\'>" +`开始给${redPacketId[j]}助力`+ "</font> \n\n"
            ...
            if (data.code === '0' && data.busiCode === '0') {
              if (data.result.luckyDrawData.actId) {
                if (data.result.luckyDrawData.redPacketId) {
                    message += "<font color=\'#FFA500\'>" +`券后9.9抽奖获得【红包】：${data.result.luckyDrawData.quota}元` + "</font> \n\n"
                  console.log(`券后9.9抽奖获得【红包】：${data.result.luckyDrawData.quota}元`);
                } else {
                    message += "<font color=\'#FFA500\'>" + `券后9.9抽奖获得【优惠券】：${data.result.luckyDrawData.discount}元：${data.result.luckyDrawData.prizeName}，${data.result.luckyDrawData.quotaDesc}` + "</font> \n\n"
...
async function red() {
  ...
  if ($.h5activityIndex && $.h5activityIndex.data && $.h5activityIndex.data.biz_code === 10002) {
    ...
    message += "<font color=\'#FFA500\'>" +`当前可拆红包个数：${$.waitOpenTimes}` + "</font> \n\n"
    if ($.waitOpenTimes > 0) {
      ...
      try {
        if (err) {
          ...
          if (data.data.success && data.data.biz_code === 0) {
            ...
            message += "<font color=\'#FFA500\'>" +`红包领取成功，获得${data.data.result.discount}元\n`+ "</font> \n\n"
    ...
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          data = JSON.parse(data);
          if (data && data.data && data.data.biz_code === 0) {
            message += "<font color=\'#FFA500\'>" +`拆红包获得：${data.data.result.discount}元`+ "</font> \n\n"
