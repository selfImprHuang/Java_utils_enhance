...
!(async () => {
  ...
  message += "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>送豆得豆</font> \n\n"
  ...
  $.openTuanList = [];
  console.log(`前${tuanLength}个账号开始开团\n`);


  

  for (let i = 0; i < cookiesArr.length && i < tuanLength; i++) {
    $.cookie = cookiesArr[i];
    $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
    $.index = i + 1;
    $.isLogin = true;
  ...
  message = "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>送豆得豆(助力)</font> \n\n"
...
})().catch((e) => { $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '') }).finally(() => {
  postToDingTalk(message)
  $.done();
});
...
async function openTuan() {
  ...
  if (!$.rewardRecordId) {
    if (!$.detail.invited) {
      ...
      message +=  "<font color=\'#778899\' size=2> " + `【京东账号${$.index}】${$.UserName} 瓜分ID:${$.rewardRecordId}` + "</font>\n\n";
...
async function helpMain() {
  ...
  for (let j = 0; j < $.openTuanList.length && $.canHelp; j++) {
    ...
    message +=  "<font color=\'#778899\' size=2> " + `${$.UserName}去助力${$.oneTuanInfo['user']}` + "</font>\n\n";
    $.detail = {};
    $.rewardRecordId = '';
    await getActivityDetail();
    if (JSON.stringify($.detail) === '{}') {
...
async function rewardMain() {
  ...
  if ($.rewardRecordId && $.detail.completed && !$.detail.rewardOk) {
    ...
    message +=  "<font color=\'#778899\' size=2> " + `奖励已领取` + "</font>\n\n";

    console.log(`奖励已领取`);
  } else {
    message +=  "<font color=\'#778899\' size=2> " + `未满足条件，不可领取奖励` + "</font>\n\n";
    ...
    $.get(options, (err, resp, data) => {
      try {
        ...
        if (data.success) {
          ...
          message +=  "<font color=\'#778899\' size=2> " + `领取豆子奖励成功` + "</font>\n\n";
        } else {
          console.log(JSON.stringify(data));
