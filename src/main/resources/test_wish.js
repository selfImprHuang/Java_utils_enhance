...
!(async () => {
  ...
  message += "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>许愿池</font> \n\n"
  ...
  message = "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>许愿池（助力）</font> \n\n"
      ...
      for (let v = 0; v < appIdArr.length; v++) {
        ...
        for (let j = 0; j < shareCode.length && $.canHelp; j++) {
          if (shareCode[j].appId === appId) {
            ...
            message += "<font color=\'#778899\' size=2> " + `${$.UserName} 去助力 ${shareCode[j].use} 的助力码 ${shareCode[j].code}` + "</font>\n\n";
...
async function jd_wish() {
  try {
    ...
    if (forNum === 0) {
      ...
      message += "<font color=\'#778899\' size=2> " + `可以抽奖${forNum}次，去抽奖` + e + "</font>\n\n";
    }
          ...
          if (safeGet(data)) {
            ...
            if (userAwardsCacheDto) {
              if (userAwardsCacheDto.type === 2) {
                ...
                message += "<font color=\'#778899\' size=2> " + `抽中：${userAwardsCacheDto.jBeanAwardVo.quantity}${userAwardsCacheDto.jBeanAwardVo.ext || `京豆`}` + "</font>\n\n";
                ...
                message += "<font color=\'#778899\' size=2> " + `抽中：${userAwardsCacheDto.couponVo.prizeName}，金额${userAwardsCacheDto.couponVo.usageThreshold}-${userAwardsCacheDto.couponVo.quota}，使用时间${userAwardsCacheDto.couponVo.useTimeRange}` + "</font>\n\n";
              } else {
                console.log(`抽中：${JSON.stringify(data)}`);
                message += "<font color=\'#778899\' size=2> " + `抽中：${JSON.stringify(data)}` + "</font>\n\n";
              }
            } else {
              $.canLottery = false
